    ...
    torch::Tensor in_edge_index)
{
  ...
  auto adj = read_graph_from_edges(edges);
  const auto m = boost::accumulate(adj | transformed([](auto &es)
                                                     { return es.size(); }),
  ...
  for (int i = 0; i < numedges; i++)
  {
    in_edge_index_ptr[0][i] = mapping[in_edge_index_ptr[0][i]];
    in_edge_index_ptr[1][i] = mapping[in_edge_index_ptr[1][i]];
  }

  // std::cout << in_edge_index << std::endl;
