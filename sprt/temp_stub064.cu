...
if(blockIdx.x == 0)
{
	...
	for(int i = threadIdx.x; i < Tsy * 3 * Tsz; i+= Block_size)
	{
		...
		AC[(0 + row) * 16 + C_offset + col] = result[row][col];
	}
#endif       
...
if(blockIdx.x == 1)
{
	...
	for(int i = threadIdx.x; i < Tsy * 3 * Tsz; i+= Block_size)
	{
		int row = i / Tsz;
		int col = i % Tsz;
		//AC[3 + row][C_offset + col] = result[row][col];
		AC[(3 + row) * 16 + C_offset + col] = result[row][col];
	}
#endif       
...
int main()
{
	...
	cnpy::NpyArray arr = cnpy::npy_load("../sprt/npys/zsy-test-graphs/g1.npy");
