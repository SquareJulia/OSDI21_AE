...
def emit_load_block_ptx(B_idx, ADDR):
    ...
    if HALF:
        ...
        new_block = block.replace("ADDR", str(ADDR)).replace(
            "OFFSET", str((channel_offset + x * IMAGE_DIM + y) * 2))
    else:
        block = LOAD_CACHE_PTX
        channel_offset = channel * IMAGE_DIM * IMAGE_DIM - IMAGE_DIM - 1
        new_block = block.replace("ADDR", str(ADDR)).replace(
            "OFFSET", str((channel_offset + x * IMAGE_DIM + y) * 4))
...
def gencode(BA, outfile, C_dim, A_blocks, C_blocks, GY, name=None):
    ...
    os.system("nvcc -arch=sm_70 -I build/include -L build/lib -w -O3 -ptx -o " + temp_ptx_file_name + " " +
    ...
    for block in range(A_blocks):
        ...
        block_ptxs = generate_from_B(
            Ny_indices, B_indices, BA, block, NY, reg_names, addresses[block], GY=GY, A_offset=A_offset)
        ptxs.append(block_ptxs)

    for i in range(len(addresses) - 1):
        if addresses[i] != addresses[i+1]:
            print(
                "The address names are different, current blurb insertion strategy won't work")
            exit()

    BLURB = blurb_half if HALF else blurb
    if HALF:
        assert IMAGE_DIM % 2 == 0
    BLURB = BLURB.replace("YLIM", str(IMAGE_DIM // 2-1)).replace("XLIM",
                                                                 str(IMAGE_DIM-1)) if HALF else BLURB.replace("LIM", str(IMAGE_DIM-1))
    ...
    if NO_RELU:
        insert_ptx(temp_ptx_file_name, outfile, ptxs,
                   relu=False, blurb=BLURB, id=IMAGE_DIM)
    else:
        insert_ptx(temp_ptx_file_name, outfile,
                   ptxs, blurb=BLURB, id=IMAGE_DIM)
